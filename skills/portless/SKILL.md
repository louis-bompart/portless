...
Most frameworks (Next.js, Express, Nuxt, etc.) respect the `PORT` env var automatically. For frameworks that ignore `PORT` (Vite, Astro, React Router, Angular, Expo, React Native), portless auto-injects the correct `--port` and `--host` CLI flags. Storybook is handled via `SBCONFIG_PORT` and `SBCONFIG_HOSTNAME` env vars.
...
Portless auto-injects `--port` and `--host` flags for frameworks that ignore the `PORT` env var: **Vite**, **Astro**, **React Router**, **Angular**, **Expo**, and **React Native**. SvelteKit uses Vite internally and is handled automatically. **Storybook** is handled via `SBCONFIG_PORT` and `SBCONFIG_HOSTNAME` env vars.
